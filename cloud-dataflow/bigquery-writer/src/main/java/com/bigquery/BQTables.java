...
public class BQTables implements Serializable {
	...
	public String dedupAddressQuery(TableId tableId) {
		return "SELECT * "  +
			"FROM ( " +
				"SELECT *, ROW_NUMBER() OVER (" +
					"PARTITION BY id)		etl_row_num " +
				"FROM `" + tableId.toString() + "`)" +
			"WHERE etl_row_num = 1;`";
	}
	...
	public Map<String, String> createTable(String datasetId, String eventtablename,
		...
		if (bigquery.getTable(tableId) == null) {
			...
			while (createRetryCount > 0) {
				try {
					ViewDefinition vd = ViewDefinition.of(dedupAddressQuery(tableId));
					TableId viewId = TableId.of(datasetId, eventtablename + "-view");
					Table createdTable = bigquery.create(
							TableInfo.of(viewId, vd)
					);
				} catch (Exception ex1) {
					System.out.println(
							"Creating view for " + tableId + " failed " + ex1.toString()
					);
				}
			}*/
		} else {
