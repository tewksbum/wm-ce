...
var msp *redis.Pool
var redisTransientExpiration = 3600 * 36
...
func ProcessOrder(ctx context.Context, m PubSubMessage) error {
	...
	for _, order := range input {
		if len(order.Billing.AddressKey) > 0 {
			if SetRedisKeyIfNotExists([]string{"order", "billto", order.Billing.AddressKey}) == 1 {
				ad := address{
		...
		for _, shipment := range order.Shipments {
			if len(shipment.AddressKey) > 0 {
				if SetRedisKeyIfNotExists([]string{"order", "shipto", shipment.AddressKey}) == 1 {
					ad := address{
...
func SetRedisKeyIfNotExists(keyparts []string) int {
	...
	if result == 1 {
		...
		_, err = ms.Do("EXPIRE", strings.Join(keyparts, ":"), redisTransientExpiration)
	}
	return result
}
