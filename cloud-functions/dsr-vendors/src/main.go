...
func Run(ctx context.Context, m *pubsub.Message) error {
	listURL := os.Getenv("VENDOR_LIST_URL")
	...
	if len(string(body)) > 10 {
		...
		log.Printf("Running update for %v vendors", len(input.Records))
		for _, rec := range input.Records {
			...
			go func(rec record) {
				...
				_, err := db.Exec("CALL sp_upsert_vendor(?,?)", rec.VendorName, rec.NetsuiteID)
				if err != nil {
