...
func Read(projectID string, csqlDSN string, r models.Record, doReadCount bool) (or wemade.OutputRecord, err error) {
	switch r.GetDBType() {
	case models.CSQL:
		or, err = csql.Read(csqlDSN, r, doReadCount)
	case models.BQ:
...
func SweepExpiredSets(projectID string, csqlDSN string, entityType string, entityBlacklist []string, entityWhitelist []string) (err error) {
	return csql.SweepExpiredSets(csqlDSN, entityType, entityBlacklist, entityWhitelist)
	// switch r.GetDBType() {
	// case models.CSQL:
