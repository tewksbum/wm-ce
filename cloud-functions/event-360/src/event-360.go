...
type EventFiberDS struct {
	...
	Identifier  MatchKeyField    `datastore:"identifier"`
	Type        MatchKeyField    `datastore:"type"`
	CampaignID  MatchKeyField    `datastore:"campaignid"`
	Browser     MatchKeyField    `datastore:"browser"`
	Channel     MatchKeyField    `datastore:"channel"`
	OS          MatchKeyField    `datastore:"os"`
	Domain      MatchKeyField    `datastore:"domain"`
	URL         MatchKeyField    `datastore:"url"`
	Location    MatchKeyField    `datastore:"location"`
	Referrer    MatchKeyField    `datastore:"referrer"`
	SearchTerm  MatchKeyField    `datastore:"searchterm"`
...
type EventOutput struct {
	Identifier MatchKeyField `json:"Id"`
...
type EventSetDS struct {
	...
	Identifier           []string       `datastore:"identifier"`
	IdentifierNormalized []string       `datastore:"identifiernormalized"`
	Type                 []string       `datastore:"type"`
	TypeNormalized       []string       `datastore:"typenormalized"`
	CampaignID           []string       `datastore:"campaignid"`
	CampaignIDNormalized []string       `datastore:"campaignidnormalized"`
	Browser              []string       `datastore:"browser"`
	BrowserNormalized    []string       `datastore:"browsernormalized"`
	Channel              []string       `datastore:"channel"`
	ChannelNormalized    []string       `datastore:"channelnormalized"`
	OS                   []string       `datastore:"os"`
	OSNormalized         []string       `datastore:"osnormalized"`
	Domain               []string       `datastore:"domain"`
	DomainNormalized     []string       `datastore:"domainnormalized"`
	URL                  []string       `datastore:"url"`
	URLNormalized        []string       `datastore:"urlnormalized"`
	Location             []string       `datastore:"location"`
	LocationNormalized   []string       `datastore:"locationnormalized"`
	Referrer             []string       `datastore:"referrer"`
	ReferrerNormalized   []string       `datastore:"referrernormalized"`
	SearchTerm           []string       `datastore:"searchterm"`
	SearchTermNormalized []string       `datastore:"searchtermnormalized"`
}

type EventGoldenDS struct {
	ID         *datastore.Key `datastore:"__key__"`
	CreatedAt  time.Time      `datastore:"createdat"`
	Identifier string         `datastore:"identifier"`
	Type       string         `datastore:"type"`
...
func Event360(ctx context.Context, m PubSubMessage) error {
	...
	MatchByKey1 := "Identifier"
	MatchByValue1 := strings.Replace(input.MatchKeys.Identifier.Value, "'", `''`, -1)

	matchedSets := []EventSetDS{}
	queriedSets := []EventSetDS{}
	...
	for _, fiber := range matchedFibers {
		...
		Fibers = append(Fibers, EventFiberDS{})
	}
	if len(FiberKeys) > 0 {
	...
	var output Event360Output
	var FiberSignatures []Signature
	output.ID = uuid.New().String()
	MatchKeyList := structs.Names(&EventOutput{})
...
func GetFiberDS(v *EventFiber) EventFiberDS {
	p := EventFiberDS{
...
func PopulateFiberMatchKeys(target *EventFiberDS, source *EventOutput) {
	KeyList := structs.Names(&EventOutput{})
	for _, key := range KeyList {
		SetEventFiberMatchKeyField(target, key, GetMatchKeyFieldFromStruct(source, key))
	}
}

func GetMatchKeyFieldFromStruct(v *EventOutput, field string) MatchKeyField {
	r := reflect.ValueOf(v)
	f := reflect.Indirect(r).FieldByName(field)
	return f.Interface().(MatchKeyField)
}

func SetEventFiberMatchKeyField(v *EventFiberDS, field string, value MatchKeyField) {
	r := reflect.ValueOf(v)
	f := reflect.Indirect(r).FieldByName(field)
	f.Set(reflect.ValueOf(value))
	LogDev(fmt.Sprintf("SetEventFiberMatchKeyField: %v %v", field, value))
}
...
func GetMatchKeyFieldFromDSFiber(v *EventFiberDS, field string) MatchKeyField {
	r := reflect.ValueOf(v)
	f := reflect.Indirect(r).FieldByName(field)
	return f.Interface().(MatchKeyField)
}
...
func PopulateSetOutputSignatures(target *EventSetDS, values []Signature) {
	KeyList := structs.Names(&Signature{})
	for _, key := range KeyList {
		SetEvent360SetOutputFieldValues(target, key, GetSignatureSliceValues(values, key))
	}
}

func PopulateSetOutputMatchKeys(target *EventSetDS, values []MatchKey360) {
	KeyList := structs.Names(&EventOutput{})
	for _, key := range KeyList {
		SetEvent360SetOutputFieldValues(target, key, GetSetValuesFromMatchKeys(values, key))
		SetEvent360SetOutputFieldValues(target, key+"Normalized", GetSetValuesFromMatchKeysNormalized(values, key))
	}
}

func PopulateGoldenOutputMatchKeys(target *EventGoldenDS, values []MatchKey360) {
	KeyList := structs.Names(&EventOutput{})
	for _, key := range KeyList {
		SetEvent360GoldenOutputFieldValue(target, key, GetGoldenValueFromMatchKeys(values, key))
	}
}
...
func SetEvent360SetOutputFieldValues(v *EventSetDS, field string, value []string) {
	r := reflect.ValueOf(v)
	f := reflect.Indirect(r).FieldByName(field)
	f.Set(reflect.ValueOf(value))
	LogDev(fmt.Sprintf("SetEvent360SetOutputFieldValues: %v %v", field, value))
}

func SetEvent360GoldenOutputFieldValue(v *EventGoldenDS, field string, value string) {
	r := reflect.ValueOf(v)
	f := reflect.Indirect(r).FieldByName(field)
	f.Set(reflect.ValueOf(value))
	LogDev(fmt.Sprintf("SetEvent360GoldenOutputFieldValue: %v %v", field, value))
}
